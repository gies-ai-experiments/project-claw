...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.projectclaw/workspace"
...
class GithubPollConfig(Base):
    """Near-real-time GitHub -> Slack by polling. Every ``interval_s`` the gateway
    checks each project repo's default branch for new commits and posts a
    plain-English summary to the project channel. Needs only repo read access
    (the GH token the daily digest already uses) — no GitHub webhook/admin."""
...
class GatewayConfig(Base):
    ...
    github_poll: GithubPollConfig = Field(default_factory=GithubPollConfig)
...
class ToolsConfig(Base):
    ...
    mindforum: MindForumToolConfig = Field(
        default_factory=lambda: _lazy_default(
            "nanobot.agent.tools.mindforum", "MindForumToolConfig"
        ),
    )
...
class Config(BaseSettings):
    ...
    def _validate_database_integrations(self) -> "Config":
        ...
        if self.database.dsn and self.memory.dsn and self.database.dsn != self.memory.dsn:

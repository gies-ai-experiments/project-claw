...
> projectclaw is built on the [nanobot](https://nanobot.wiki) agent framework. The framework's Python package directory and SDK class still use the upstream `nanobot` name; the user-facing CLI (`projectclaw`) and config directory (`~/.projectclaw/`) have been rebranded.
...
The bot also posts two recurring updates on schedule (configurable per channel): a **daily standup** (Tue–Fri 9am — open PRs + last 24h merges) and a **weekly summary** (Mon 9am — last 7d merges, aging open PRs, issue activity).
...
projectclaw is a fork of the [nanobot](https://nanobot.wiki) agent framework by [HKUDS](https://github.com/HKUDS/nanobot). The upstream framework provides the agent loop, channel adapters, provider integrations, and skill system that projectclaw extends with multi-project scoping, GitHub status integration, and the Granola tool.
